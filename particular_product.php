...
// --- Determine WHERE clause and page title ---
if ($special) {
...
<head>
  <meta charset="UTF-8">
  <title><?= $page_title ?> - CraftAura</title>
  ...
  <div class="max-w-7xl mx-auto py-10 px-4">
      ...
      <?php while($row = $result->fetch_assoc()): ?>
        <div class="bg-white p-4 rounded-lg shadow hover:shadow-md transition cursor-pointer" onclick="window.location='product.php?id=<?= $row['id'] ?>'">
          ...
          <?php if ($row['special_offer']): ?>
            <span class="inline-block mt-2 px-3 py-1 text-xs bg-yellow-300 text-gray-800 font-semibold rounded-full">Special Offer</span>
          <?php endif; ?>
        </div>
      <?php endwhile; ?>
    </div>
    ...
    <div class="flex justify-center mt-8 space-x-2">
      ...
      <?php for ($i = 1; $i <= $total_pages; $i++): ?>
        <a href="<?= $base_url ?>&page=<?= $i ?>" class="px-3 py-1 rounded border <?= $page == $i ? 'bg-blue-500 text-white' : 'bg-white text-blue-500' ?>"><?= $i ?></a>
      <?php endfor; ?>
    </div>
  </div>
</body>
